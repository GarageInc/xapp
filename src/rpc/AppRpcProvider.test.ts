...
describe('AppRpcProvider', () => {
  ...
  test('constructor initializes with valid providers', () => {
    expect(() => new AppRpcProvider(SupportedChainId.MAINNET, mockProviders)).not.toThrow()
  })

  test('constructor throws with empty providers array', () => {
    expect(() => new AppRpcProvider(SupportedChainId.MAINNET, [])).toThrow('providers array empty')
  })

  test('constructor throws with network mismatch', () => {
    mockProviders[0].network.chainId = 2
    expect(() => new AppRpcProvider(SupportedChainId.MAINNET, mockProviders)).toThrow('networks mismatch')
  })

  test('constructor throws with invalid providers', () => {
    mockIsProvider.mockReturnValueOnce(false)
    expect(() => new AppRpcProvider(SupportedChainId.MAINNET, [{} as JsonRpcProvider])).toThrow('invalid provider')
  })

  test('handles sendTransaction', async () => {
    const hash = '0x123'
    mockProvider1.sendTransaction.mockResolvedValue({ hash } as TransactionResponse)
    const provider = new AppRpcProvider(SupportedChainId.MAINNET, [mockProvider1])
  ...
  test('should sort faster providers before slower providers', async () => {
    ...
    const appRpcProvider = new AppRpcProvider(SupportedChainId.MAINNET, mockProviders)
